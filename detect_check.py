...
out = model.generate(**enc, max_new_tokens=1500, do_sample=True, top_p=0.9, temperature=0.8, pad_token_id=tok.eos_token_id)
...
print(f"\n✅ Generated {len(sents)} sentences!")
print("\nFirst 3 sentences:")
for i, s in enumerate(sents[:3]):
    print(f"  {i}: {s[:100]}...")

if len(sents) >= 10:
    print("\n🎉 SUCCESS! Model is generating properly!")
else:
    print("\n⚠️ Only generated", len(sents), "sentences")
